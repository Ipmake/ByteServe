...
export default async function purgeOldObjects() {
    ...
    for (const obj of dbObjects) {
        objectsMap.set(obj.id, obj.bucketId);
    }
    ...
    for (const bucketId of bucketDirs) {
        const bucketPath = path.join(storageDir, bucketId);
        ...
        for (const objectFile of objectFiles) {
            ...
            if (!objectsMap.has(objectId) || objectsMap.get(objectId) !== bucketId) {
