...
export async function handleMove(req: express.Request, res: express.Response) {
    try {
        ...
        let sourceObject: Prisma.ObjectGetPayload<Prisma.ObjectDefaultArgs> | null = null;
        ...
        for (const part of pathParts) {
            ...
            if (!sourceObject) return res.status(404).send('Source not found');
        ...
        for (const part of destParts) {
            const parentObj: Prisma.ObjectGetPayload<Prisma.ObjectDefaultArgs> | null = await prisma.object.findFirst({
                where: {
                    bucketId: bucketObj.id,
            ...
            if (!parentObj) return res.status(404).send('Destination parent not found');
        ...
        while (tempParentId) {
            const parentObj: Prisma.ObjectGetPayload<Prisma.ObjectDefaultArgs> | null = await prisma.object.findUnique({
                where: { id: tempParentId },
            });
            if (parentObj) {
