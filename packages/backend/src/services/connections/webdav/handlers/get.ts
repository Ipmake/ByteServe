...
export async function handleGet(req: express.Request, res: express.Response) {
    try {
        ...
        if (rangeHeader) {
            ...
            const fileStream = fs.createReadStream(storagePath, { start, end, highWaterMark: 1024 * 1024 });
            ...
            let bytesStreamed = 0;
            const startTime = Date.now();
            const fileStream = fs.createReadStream(storagePath, { highWaterMark: 1024 * 1024 });
