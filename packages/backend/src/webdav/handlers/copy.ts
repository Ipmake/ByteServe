...
export async function handleCopy(req: express.Request, res: express.Response) {
    try {
        ...
        const sourceStoragePath = path.join(process.cwd(), 'storage', bucket, ...sourceFolderPath, sourceObject.id);
        ...
        const newObjectId = crypto.randomBytes(16).toString('hex');
        const destStoragePath = path.join(process.cwd(), 'storage', bucket, ...destFolderPath, newObjectId);
        
        // Delete existing destination if overwrite is true
        if (existingDest) {
            const existingPath = path.join(process.cwd(), 'storage', bucket, ...destFolderPath, existingDest.id);
        ...
        const destStorageDir = path.dirname(destStoragePath);
        if (!fs.existsSync(destStorageDir)) {
            fs.mkdirSync(destStorageDir, { recursive: true });
        }
        
        // Copy the physical file/folder
        if (fs.existsSync(sourceStoragePath)) {
            if (sourceObject.mimeType === 'folder') {
                // Copy folder recursively
                fs.cpSync(sourceStoragePath, destStoragePath, { recursive: true });
            } else {
                // Copy file
                fs.copyFileSync(sourceStoragePath, destStoragePath);
            }
        }
        
        // Copy database object
        async function copyObject(srcObj: any, newParentId: string | null, newName: string, newId: string) {
            ...
            if (srcObj.mimeType === 'folder') {
                ...
                for (const child of children) {
                    ...
                    await copyObject(child, newObj.id, child.filename, childId);
                }
            }
        }
        
        await copyObject(sourceObject, destParentId, newFilename, newObjectId);
